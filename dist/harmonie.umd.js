(function (global, factory) {
  ...
  (global = global || self, global.harmonie = factory());
  ...
  */
    ...
    parseGML(gml) {
      ...
      if (utils.getSafe(() => features)) {
        ...
        features.forEach(field => {
          ...
          const polygon = helpers.polygon([coordinates], {number: id, year: year});
  ...
  var helpers$1 = {
    ...
    toGeoJSON: function toGeoJSON(gml) {
      var _this = this;

      // convert gml to json
      var json = parse$1.xml(gml.replace(/&lt;/g, '<').replace(/&gt;/g, '>'));
      var flatJson = this.flatten(json);
    ...
    toCoordinates: function toCoordinates(string, keepProjection) {
      var numbers = string.split(/\s+/g).map(function (s) {
        return Number(s);
      });
      var coords = this.toPairs(numbers);
  ...
  function queryComplete(query, requiredProps) {
    return requiredProps.reduce(function (incomplete, curProp) {
      if (curProp in query) return '';
      return incomplete + "Missing property ".concat(curProp, " for state ").concat(query.state, ". ");
    }, '');
  }

  var Field = function Field(properties) {
    _classCallCheck(this, Field);

    this.id = properties.id || 'String';
    this.referenceDate = properties.referenceDate;
    this.NameOfField = properties.NameOfField || "Unbenannt ".concat(properties.NumberOfField);
          ...
          switch (_context.prev = _context.next) {
            ...
            case 3:
              ...
              return _context.abrupt("return", parzellen.reduce(function (acc, p) {
                ...
                acc.push(new Field({
                  id: "harmonie_".concat(count, "_").concat(hnf['fa:flik']),
                  referenceDate: applicationYear,
                  NameOfField: '',
                  // seems to be unavailable in Agrarantrag-BB export files?,
                  NumberOfField: Math.floor(hnf['fa:teilflaechennummer']),
                  Area: hnf['fa:groesse'],
                  FieldBlockNumber: hnf['fa:flik'],
                  PartOfField: 'a',
                  SpatialData: helpers$1.toGeoJSON(hnf['fa:geometrie']),
                  LandUseRestriction: '',
                  Cultivation: {
                    PrimaryCrop: {
                ...
                count++; // go on with field (buffer) strips

                var strfFlaechen = p['fa:teilflaechen']['fa:streifen_flaechen']; // return only main area if no field strips are defined

                if (!strfFlaechen) return acc; // convert to array structure if only one buffer strip is defined

                if (!Array.isArray(strfFlaechen['fa:streifen'])) {
                  strfFlaechen['fa:streifen'] = [strfFlaechen['fa:streifen']];
                }

                strfFlaechen['fa:streifen'].forEach(function (stf, j) {
                  count++;
                  acc.push(new Field({
                    id: "harmonie_".concat(count, "_").concat(stf['fa:flik']),
                    referenceDate: applicationYear,
                    NameOfField: '',
                    // seems to be unavailable in Agrarantrag-BB export files?,
                    ...
                    PartOfField: helpers$1.toLetter(j + 1),
                    SpatialData: helpers$1.toGeoJSON(stf['fa:geometrie']),
                    LandUseRestriction: '',
                    Cultivation: {
                      PrimaryCrop: {
  ...
  function harmonie(query) {
    ...
    if (!state) {
      throw new Error('No property "state" given, required to be in ' + 'ISO 3166-2 UTF-8 string format (e.g. "DE-NW")');
    }
    ...
    switch (state) {
      ...
      default:
        throw new Error("No such state as \"".concat(state, "\" according to ISO 3166-2 in Germany.\""));
    }
  }
